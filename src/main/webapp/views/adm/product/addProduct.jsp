...
<form id="addProduct" style="font-size: 0;" onsubmit="return false;" method="post" enctype="multipart/form-data">
	...
	<div class="product">
		...
		<div  class="item">
			...
			<div class="rightItem">
				<span class="layui-btn" @click.stop="openFile"><i class="layui-icon">&#xe67c;</i>{{imgUrl == '' ? '选择图片':'替换图片'}}</span>
				<span :title="getImgName()">{{getImgName()}}</span>
				<input @change="imgChange" ref='img' type="file" name="img" style="opacity: 0;" :accept="imgSuffixs.join(',')">
			</div>
...
<script type="text/javascript">
	new Vue({
		...
		watch:{
			...
			imgRealPath: function(val){
				console.log(val);
			},
		},
		methods:{
			getImgName: function(){
				var imgRealPath = this.imgRealPath;
				var imgName =!!imgRealPath && imgRealPath.substring(imgRealPath.lastIndexOf("\\")+1);
				if(!!imgName){
					return imgName;
				} else {
					return "未选择图片";
			...
			imgChange: function($event){
				this.imgRealPath = $event.target.value
				console.log(event);
			},
			openFile: function(){
				this.$refs.img.click();
